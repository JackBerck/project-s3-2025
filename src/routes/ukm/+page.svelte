...
<svelte:head>
	<title>UKM | Soedirman Student Summit (S3) 2025</title>
</svelte:head>
...
>
	...
	<div class="absolute inset-0 bg-[url('/img/patterns/batik-1.webp')] opacity-30"></div>
	...
	<div class="relative z-10 container max-w-screen-xl">
		<div class="mb-6 flex flex-col items-center justify-center md:mb-8">
			...
			{#if elementsVisible.heroDescription}
				...
				>
					Unit Kegiatan Mahasiswa atau yang biasa kita kenal dengan UKM merupakan wadah berkembang,
					belajar bersama, berbagi informasi dan saling tolong menolong bagi sobat gensoed yang
					memiliki minat dan bakat serupa.
				</p>
			{/if}
		</div>

		{#if elementsVisible.searchForm}
			<div class="flex justify-center" in:fade={{ duration: 800, delay: 0 }}>
...
>
	<div class="container max-w-screen-xl">
		{#if elementsVisible.cardContainer}
			{#if filteredUkm.length > 0}
				...
				>
					{#each filteredUkm as item, index}
						<div
							class="ukm-card mb-6 break-inside-avoid"
			...
			{:else}
				<!-- Empty State -->
				<div class="py-16 text-center" in:fade={{ duration: 800, delay: 0 }}>
					<div class="mb-6">
						<svg
					...
					<p class="mb-6 text-gray-500" in:fly={{ y: 20, duration: 600, delay: 500 }}>
